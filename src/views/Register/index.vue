<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>手机号:</label>
        <input placeholder="请输入你的手机号" v-model="username" name="username" v-validate="{required:true, regex: /^1\d{10}$/ }" :class="{invalid:errors.has('username')}" />
      ...
      <div class="content">
        ...
        <button style="width:100px;height:38px" @click="getCode(username)">获取验证码</button>
      ...
      <div class="controls">
        <input name="agree" type="checkbox" checked v-validate="{required:true, tongyi:true }" :class="{invalid:errors.has('agree')}">
        <span>同意协议并注册《尚品汇用户协议》</span>
        <span class="error-msg">{{errors.first('agree')}}</span>
      </div>
      <div class="btn">
        <button @click="userRegister(username,password,code)">完成注册</button>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      async getCode(username){
        ...
        let result = await this.$API.reqGetcode(username)
        this.code = result.data
      },

      // 用户注册
      async userRegister(username,password,code){
        const success = await this.$validator.validateAll();
        if(success){
          let result = await this.$API.reqRegister({username,password,code})
          if(result.status==0){
            alert('注册成功')
